...
- [ ] **IPv4 / ICMP / TCP スタックの完成**
  - IP アドレスの割り当てと Ping (ICMP Echo) 応答機構の実装、および TCP ハンドシェイク処理の組み込み。
- [ ] **Singularity Web Server**
  - SLS（単一レベルストア）と連携し、HTTPリクエストに対して永続オブジェクトを NIC に直接 DMA 送信する「コンテキストスイッチ・ゼロ、コピー・ゼロ」の究極のWebサーバを稼働。
