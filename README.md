<div align="center">
  ...
  <p><strong>Version v0.0.1-6-4 "Stateful Ring 0 JIT Firewall"</strong></p>
  <p>
...
v0.0.1-6-4 では、動的コードをRing 0特権空間で安全に実行するサンドボックス「ASH (Application-Specific Safe Handlers)」が、ついにステート（状態）を保持できる**「Stateful Ring 0 JIT Firewall」**へと究極進化しました。JITコンパイラが専用のPersistent State（永続状態メモリ）領域へゼロコストでアクセスできるようになり、TCPコネクションの追跡やSYNフラッド防御といった高度な「ステートフル・インスペクション」が単なるフィルタ以上の次元で可能になります。また、コンパイル時にオフセットが確定しない動的メモリアクセスに対しても、**MBC (Mask-Based Bounds Checking)** を導入することで、分岐予測ミスを一切引き起こさない100%安全かつブランチレスなゼロコスト境界チェックを実現しています。VMM（仮想メモリマッパー）と統合された**「W^X (Write XOR Execute) Enforcer」**の多層防壁も健在です。
...
### 1. Stateful Ring 0 JIT Compiler & Zero-Cost MBC 【v0.0.1-6-4 目玉機能 ✨】
特権空間（Ring 0）の内部で稼働する、超高速な動的ネイティブコード生成エンジンです。パケットフィルタやNATの書き換えといった外部の未検証コード（ASHバイトコード）を、実行時にx86_64の純粋なマシン語へ直接トランスパイルします。v0.0.1-6-4より、JITは単一パケットの検査だけでなく、永続化された状態（State）を跨いで読み書きできる**Stateful Firewall**機能を手に入れました。
最大の特異点は「境界チェックの完全ブランチレス化 (MBC)」です。領域外メモリアクセスを防ぐため、実行時の分岐（`if` 文）を挿入するのではなく、トランスパイル段階でレジスタに対する物理的なビットマスク（`and reg, 0x3F`）を強制挿入します。これにより、コンパイル時にオフセットが不明な動的パケットメモリアクセスや、動的ステートメモリアクセスであっても、OSを絶対にクラッシュさせない100%の安全性を保ちながら、C言語のハードコードと同等以上の光速実行を実現しています。

### 2. W^X (Write XOR Execute) Memory Enforcer 【v0.0.1-6-4 目玉機能 ✨】
...
Copyright (c) 2026 pangea-ring0-os developers. <a href="https://argleton-ghoti.com/">https://argleton-ghoti.com/</a>