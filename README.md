...
- [ ] **v0.0.2-4: Ring 0 POSIX エミュレーションとレガシーバイナリ互換**
  - 通常 Ring 3 から発行されるシステムコール (`syscall`) を Ring 0 空間内でトラップ・エミュレートし、既存の C言語プログラム（Linuxバイナリ等）を PangeaOS の SIPs 内部で安全に稼働させる「サンドボックス型互換レイヤー」の実装。
