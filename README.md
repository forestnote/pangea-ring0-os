<div align="center">
  ...
  <p><strong>Version v0.0.1-4 "Multicore Singularity Awakened"</strong></p>
  <p>
...
v0.0.1-4 では、これまでの**「Local APIC」**や**「Async-Await タスクスケジューラ」**、**「True Mesh Allocator」**といった強固なシングルコア基盤の上に、ついに**「Symmetric Multiprocessing (SMP) の初期化」**が実装されました。すべてのCPUコア（Application Processors）が目覚め、真の並列処理へ向けた準備が完了しています。
...
### 7. SMP Ignition & Processor Parking (Phase 4 進行中)
レガシーな INIT-SIPI-SIPI シーケンスを手動で叩くのではなく、Limine ブートローダの最新プロトコル (`MpRequest`) を利用してすべてのサブコア (Application Processors) をセーフティに初期化。各コアは起動直後に自身の APIC ID を自己認識し、Ring 0 空間の専用エントリポイント (`ap_main`) にて安全に待機（hltループ）します。これにより、カーネルは「いつ・どのコアに・何のタスクを割り当てるか」という絶対的な指揮権を握りました。
...
**Phase 4: 未踏領域 (Upcoming / In Progress)**
- [x] マルチコアプロセッシング (SMP: Symmetric Multiprocessing) の初期化 (v0.0.1-4 にて完了 🎉)
- [ ] 各コアごとの GDT / TSS 独立化および Local APIC タイマーの分散化
